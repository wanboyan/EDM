...
from equi_diff_models.diffusion import DiffusionModel_v3,DiffusionModel_occ,DiffusionModel_score,DiffusionModel_v4
from equi_diff_models.diffusion_ddm import DiffusionModel_ddm
from equi_diff_models.archs.diffusion_arch import DiffusionNet_v4,DiffusionNet_occ,DiffusionNet_res,DiffusionNet_v3,DiffusionNet_v5,DiffusionNet_v6
...
class PIPS_s(nn.Module):
    def __init__(self):
        ...
        if FLAGS.use_occ:
            self.diffusion_model = DiffusionModel_occ(model=DiffusionNet_occ())
        elif FLAGS.use_score:
            self.diffusion_model = DiffusionModel_score(model=DiffusionNet_v4())
        elif FLAGS.use_ddm:
            self.diffusion_model = DiffusionModel_ddm(model=DiffusionNet_v4())
        elif FLAGS.use_pointe:
            self.diffusion_model = DiffusionModel_v3(model=DiffusionNet_v5())
        elif FLAGS.use_michel:
            self.diffusion_model = DiffusionModel_v4(model=DiffusionNet_v6())
        else:
            self.diffusion_model = DiffusionModel_v3(model=DiffusionNet_v4())
    ...
    def forward(self, depth, def_mask, camK,gt_2D, gt_R, gt_t, aug_rt_t=None, aug_rt_r=None,aug_scale=None,
                latent_code=None,cls_vector=None,occ_r_vector=None,
                sphere_points=None,model_size=None,model_points=None):
        PC = PC_sample(def_mask, depth, camK, gt_2D)
        bs=PC.shape[0]


        count_conv=sum(p.numel() for p in self.backbone1.parameters() if p.requires_grad)
        count_diff=sum(p.numel() for p in self.diffusion_model.model.causal_transformer_latent.parameters() if p.requires_grad)
        ...
        if FLAGS.stage==2:
            with torch.no_grad():
                self.backbone1.eval()
                final_fea,final_point,fea_dict= self.backbone1(pc_center,return_fuse=True)

            if FLAGS.use_occ:
                # gt_green_v, gt_red_v = get_gt_v(gt_R)
                # r_vector=torch.cat([gt_green_v,gt_red_v],dim=1)
                r_vector=torch.einsum('bij,bqj->bqi',gt_R,occ_r_vector)
                diff_loss, loss_100_latent,loss_100_r,loss_100_trans,loss_100_scale, loss_1000_latent,loss_1000_r,loss_1000_trans,loss_1000_scale \
                    =self.diffusion_model.diffusion_model_from_latent_ts(latent_code, r_vector,gt_t-center.squeeze(1),
                                                                     nocs_scale,cond=final_fea, point=final_point,cls_vector=cls_vector)
                ...
                diff_loss, loss_100_latent,loss_100_trans,loss_100_scale, loss_1000_latent,loss_1000_trans,loss_1000_scale\
                    =self.diffusion_model.diffusion_model_from_latent_ts(gt_latent_code, gt_t-center.squeeze(1),
                                                                         nocs_scale,cond=final_fea, point=final_point,cls_vector=cls_vector)

                loss_100_r=loss_1000_r=0
            loss_dict={'loss':diff_loss,
                       'loss_100_latent':loss_100_latent,
                       'loss_100_r':loss_100_r,
                       'loss_100_trans':loss_100_trans,
                       'loss_100_scale':loss_100_scale,
                       'loss_1000_latent':loss_1000_latent,
                       'loss_1000_r':loss_1000_r,
        ...
        params_lr_list.append(
            {
                ...
                "lr": float(FLAGS.lr)*FLAGS.lr_diff,
            }
        )
        params_lr_list.append(
            {
