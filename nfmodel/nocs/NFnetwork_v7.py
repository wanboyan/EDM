...
import os
FLAGS = flags.FLAGS
from nfmodel.part_net_v7 import MyNeuron,GCN3D_segR,Rot_red,Rot_green,MyQNet,Pose_Ts,Point_center_res_cate,Equi_gcn,Equi_gcn2
...
class NFPose(nn.Module):
    def __init__(self):
        ...
        if FLAGS.backbone=='gcn':
            self.backbone1=GCN3D_segR(support_num= FLAGS.gcn_sup_num, neighbor_num= FLAGS.gcn_n_num)
        elif FLAGS.backbone=='neuron':
            self.backbone1=MyNeuron()
        elif FLAGS.backbone=='gcn_equi':
            self.backbone1=Equi_gcn(FLAGS.equi_neighbor_num)
        else:
