...
class NFPose(nn.Module):
    ...
    def forward(self, depth, obj_id, camK,
        ...
        if  FLAGS.use_inter==0:
            if FLAGS.decoder_only:
                PC=torch.bmm(model_point*nocs_scale,gt_R.permute(0,2,1))+gt_t.reshape(-1,1,3)
                PC_1 = PC
                gt_s_1 = gt_s
            else:
            ...
            pred_fsnet_list_1,bin_list_1= \
                self.one_branch(PC_1,sdf_points,pad_points,mean_shape,gt_s_1,gt_R,gt_t,sym,cat_name)
            if FLAGS.decoder_only:
                fsnet_loss_1={
                    'Rot1': 0,
                    'Rot2': 0,
            ...
            PCs = proj(def_mask, depth, camK, gt_2D)
            for i in range(len(PCs)):
                PCs[i]=PCs[i].detach()
                PC_1, PC_2, gt_s_1,gt_s_2,gt_R, gt_t, = \
                    self.branch_data_augment(PCs, gt_R, gt_t, gt_s, mean_shape, sym, aug_bb,
                                                                          aug_rt_t, aug_rt_r, model_point, nocs_scale, obj_id)
                # show_open3d(PC_1[0].cpu().detach().numpy(),PC_2[0].cpu().detach().numpy())
                gt_green_v, gt_red_v = get_gt_v(gt_R)
                gt_fsnet_list_1 = {
                    'Rot1': gt_green_v,
                    'Rot2': gt_red_v,
                ...
                name_fs_list=gt_fsnet_list_1
                pred_fsnet_list_1,bin_list_1= \
                    self.one_branch(PC_1,sdf_points,pad_points,mean_shape,gt_s_1,gt_R,gt_t,sym,cat_name,gt_s)
                pred_fsnet_list_2,bin_list_2= \
                    self.one_branch(PC_2,sdf_points,pad_points,mean_shape,gt_s_2,gt_R,gt_t,sym,cat_name,gt_s)

                fsnet_loss_1=self.loss_fs_net(name_fs_list, pred_fsnet_list_1, gt_fsnet_list_1, sym)
                fsnet_loss_2=self.loss_fs_net(name_fs_list, pred_fsnet_list_2, gt_fsnet_list_2, sym)

                inter_dict=self.loss_inter(pred_fsnet_list_1,pred_fsnet_list_2,bin_list_1,bin_list_2,sym)

                loss_nocs_1=self.loss_nocs(bin_list_1,sym)
                loss_nocs_2=self.loss_nocs(bin_list_2,sym)
