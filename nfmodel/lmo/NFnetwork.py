...
class PIPS_s(nn.Module):
    ...
    def forward(self, depth, def_mask, camK,gt_2D,
        ...
        pred_scale=torch.ones_like(model_size)[:,0]
        ...
        if len((sym_trans))<=1:
            ...
            # gt_query_nocs_aug=F.pad(gt_query_nocs,(0,1),'constant',1)
            # sym_query_nocs_aug=torch.einsum('sij,bpj->bspi',sym_trans,gt_query_nocs_aug.reshape(bs,query_num,-1))
            # model_points_aug=F.pad(model_points,(0,1),'constant',1)
            # sym_model_points=torch.einsum('sij,bpj->bspi',sym_trans,model_points_aug)[:,:,:,:3]
            #
            #
            # sym_query_nocs=sym_query_nocs_aug[:,:,:,:3]
            # r=torch.norm(sym_query_nocs,dim=-1,keepdim=True)
            # x=sym_query_nocs[:,:,:,0:1]
            # y=sym_query_nocs[:,:,:,1:2]
            # z=sym_query_nocs[:,:,:,2:3]
            # theta=torch.acos(z/r)
            # phi = (torch.atan2(y, x)+2*torch.pi) % (torch.pi*2)
            # key=phi*100+theta
            # min_idx=torch.min(key,dim=1,keepdim=True)[1]
            # gt_query_nocs_pick=torch.gather(sym_query_nocs,1,min_idx.repeat(1,1,1,3)).squeeze(1)
            #
            # diff=pred_coord-gt_query_nocs_pick.reshape(bs*query_num,-1)
            # nocs_loss,var_loss=self.loss_coord(diff,pred_log_stds,pred_var_R)
            # NOCS_loss = {'nocs_loss':nocs_loss.mean(),'var_loss':var_loss.mean(),'c_nocs_loss':0,
            #              'c_var_loss':0}
            # show_open3d(model_points[0].cpu().numpy(),model_points[0].cpu().numpy())
            gt_query_nocs[:,:,0]=torch.abs(gt_query_nocs[:,:,0])
            gt_query_nocs[:,:,1]=torch.abs(gt_query_nocs[:,:,1])
            diff=pred_coord-gt_query_nocs.reshape(bs*query_num,-1)
            nocs_loss,var_loss=self.loss_coord(diff,pred_log_stds,pred_var_R)
            NOCS_loss = {'nocs_loss':nocs_loss.mean(),'var_loss':var_loss.mean(),'c_nocs_loss':0,
                         'c_var_loss':0}
