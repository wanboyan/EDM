...
class VecDGCNN_att(nn.Module):
    def __init__(
        ...
        for i in range(self.num_layers):
            ...
            if i == 0:
                self.V_list.append(VecLNA(1, feat_dim[0], mode="so3", act_func=act_func))
            else:
                self.V_list.append(
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            ...
            src_nn_f = self.get_graph_feature(
                src_f=src_f, dst_f=dst_f, k=num_knn, src_xyz=src_xyz, dst_xyz=dst_xyz, first_layer=(i==0),
...
class MyVecDGCNN_att(nn.Module):
    def __init__(
            self,
            c_dim=256,
            num_layers=5,
            feat_dim=[128, 128, 256, 256, 512],
            down_sample_layers=[2, 4],
            down_sample_factor=[4, 4],
        ...
        for i in range(self.num_layers):
            ...
            self.fc_list.append(VecLinear(feat_dim[i], feat_dim[i], mode="so3"))
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            ...
            if self.use_res_global_conv and i >= self.res_global_start_layer:
                ...
                dst_f = torch.cat([dst_f, dst_f_global[..., None].expand_as(dst_f)], 1)
                dst_f = self.global_conv_list[i - self.res_global_start_layer](dst_f)
            inv_f=self.fc_list[i](dst_f)
            inv_f=channel_equi_vec_normalize(inv_f)
            dst_f_norm=channel_equi_vec_normalize(dst_f)
            inv_f=(inv_f.permute(0,-1,1,2)*dst_f_norm.permute(0,-1,1,2)).sum(-1)

            feat_dict["conv{}".format(i)]={'pos':dst_xyz.squeeze(1).permute(0,2,1),'fea':inv_f}

            src_xyz, src_f = dst_xyz, dst_f

        x = self.conv_c(dst_f)
        x_mean = x.mean(dim=-1, keepdim=False)
        pred_scale = x_mean.norm(dim=-1).mean(1) * self.scale_factor

        return feat_dict,pred_scale,x
