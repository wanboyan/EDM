...
class VNLinear(nn.Module):
    ...
    def forward(self, x):
        out = torch.einsum('... i c, o i -> ... o c', x, self.weight)
...
class VNReLU(nn.Module):
    ...
    def forward(self, x):
        q = torch.einsum('... i c, o i -> ... o c', x, self.W)
        k = torch.einsum('... i c, o i -> ... o c', x, self.U)
...
class VNMaxPool(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        q_para_len = (q * k).sum(dim=2, keepdim=True)

        if mask!=None:
            mask=mask.bool()
            mask=mask[:,None,None,:].repeat(1,x.shape[1],1,1)
            q_para_len[~mask]=-1
...
class Vecnorm(nn.Module):
    def __init__(
            ...
            dim,):
        super().__init__()
        self.normalization=nn.BatchNorm1d(dim)
