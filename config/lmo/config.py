...
import config.lmo.pips_s_config
import config.lmo.fix_config
import numpy as np
...
np.int=np.int_
# eval
flags.DEFINE_string('eval_out', 'lmo', '')

flags.DEFINE_float('stage',2,'')

flags.DEFINE_string('pic_save_dir', 'lmo_eval_per_cat', '')
...
# flags.DEFINE_string('qnet_version','v6','')
...
flags.DEFINE_string('dataset_dir', '/data_sata/lmo', 'path to the dataset')
...
flags.DEFINE_integer('total_epoch', 30, 'total epoches in training')
...
flags.DEFINE_string('model_save', 'output/lmo', 'path to save checkpoint')


# resume
flags.DEFINE_integer('resume',0, '1 for resume, 0 for training from the start')
flags.DEFINE_string('resume_dir', '/data_nvme/trained_models/equi_lmo','')
flags.DEFINE_string('resume_model_name', 'model_64.pth','')

flags.DEFINE_integer('resume_point', 19, 'the epoch to continue the training')
