...
class QNet_equi_an(nn.Module):
    def __init__(self, cfg_file, qnet_handcrafted_cfg=None):
        ...
        for i in range(self.num_levels):
            ...
            decoder_layers = nn.ModuleList([
                transformer.TransformerDecoderLayer_equi_an(
            ...
            encoder_layers = nn.ModuleList([
                transformer.TransformerEncoderLayer_equi_an(
                    self.q_version, self.q_target_chn_per_level[i], self.q_head_per_level[i],
                    self.q_dim_feedforward_per_level[i], self.q_dropout_per_level[i])
        ...
        weight=torch.from_numpy(np.array([1.0e10,1]))
    ...
    def qnet(self, data_dict, query_features, query_pos,
        ...
        return query_features.view(batch_size, -1, self.q_target_chn_per_level[level_idx],an)
    ...
    def forward(self, data_dict,pred_scale=None):
        ...
        for i in range(self.num_levels):
            ...
            query_features = cur_support_features.new_zeros(
                (batch_size, query_positions.shape[1], self.q_target_chn_per_level[i]*an))
        ...
        if FLAGS.fea_type=='a5':
            vert_value_1=query_features[:,:,:,self.face_to_cube[:,0]]*torch.relu(query_features[:,:,:,self.face_to_cube[:,0]]-query_features[:,:,:,self.face_to_cube[:,1]])
            vert_value_1=torch.einsum('b c q n , n i->b c q i',vert_value_1,self.face_normal)
            query_features=vert_value_1
            # vert_value_1=query_features[:,:,:,self.face_to_cube]*self.weight.float()
            # vert_value_1=torch.relu(vert_value_1.sum(-1))
            # vert_value_1=torch.einsum('b c q n , n i->b c q i',vert_value_1,self.face_normal)
            # query_features=vert_value_1

        if FLAGS.fea_type=='a6':
            vert_color_1=query_features[:,:,:,self.color_com][:,:,:,:,0]
            vert_color_1=torch.einsum('bqcv, vi -> bqcvi',vert_color_1,self.vs)
            vert_color_pair_1=vert_color_1[:,:,:,self.color2v,:]
            vert_color_roll_1=query_features[:,:,:,self.color_com][:,:,:,:,1:].detach()

            vert_color_roll_1=vert_color_roll_1[:,:,:,:,self.roll]*self.weight
            vert_color_roll_1=vert_color_roll_1.sum(-1).max(-1)[0].detach()

            vert_color_pair_index_1=torch.max(vert_color_roll_1[:,:,:,self.color2v],dim=-1,keepdim=True)[1].detach()
