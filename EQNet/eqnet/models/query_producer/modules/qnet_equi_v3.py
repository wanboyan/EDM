...
class QNet_equi_v3(nn.Module):
    def __init__(self, cfg_file, qnet_handcrafted_cfg=None):
        ...
        for i in range(self.num_levels):
            ...
            decoder_layers = nn.ModuleList([
                transformer.TransformerDecoderLayer_equi_v3(
            ...
            encoder_layers = nn.ModuleList([
                transformer.TransformerEncoderLayer_equi_v3(
                    self.q_version, self.q_target_chn_per_level[i], self.q_head_per_level[i],
                    self.q_dim_feedforward_per_level[i], self.q_dropout_per_level[i])
        ...
        self.length_enc = GaussianRadialBasis(dim = 64, max_val=2)

        self.length_range=(0.01,0.05,None,None)

        self.edge_scalars_pre_linear=torch.nn.Sequential(
    ...
    def qnet(self, data_dict, query_features, query_pos,
        ...
        ca_edge_scalars=self.length_enc(ca_length.reshape(-1))
        ca_edge_scalars=self.edge_scalars_pre_linear(ca_edge_scalars)
        ...
        sa_edge_scalars=self.length_enc(sa_length.reshape(-1))
        sa_edge_scalars=self.edge_scalars_pre_linear(sa_edge_scalars)
        ...
        for i in range(num_q_layers):
            ...
            query_features = self.q_decoder[level_idx][i](
                query_features, support_key_features,ca_edge_sh,ca_edge_scalars)
            ...
            if i != (num_q_layers - 1):
                support_features = self.q_encoder[level_idx][i](
                    support_features,sa_key_features,sa_edge_sh,sa_edge_scalars)
                # print(1)
