...
class MultiheadAttention_equi(Module):
    ...
    def __init__(
        ...
        super(MultiheadAttention_equi, self).__init__()
        self.embed_dim = embed_dim
        self.num_heads = num_heads
        self.dropout = dropout
        self.head_dim = embed_dim*3 // num_heads
        assert (
            self.head_dim * num_heads == self.embed_dim*3
    ...
    def _proj_qkv(self, t, start, end):
        ...
        t=torch.einsum('pci,dc->pdi',t,_w)
        return t.reshape(t.shape[0],-1)

    def _proj_out(self, x):

        x=torch.einsum('pci,dc->pdi',x,self.out_proj_weight)
        return x
    ...
    def forward(
        ...
        attn_output = attn_output.view(total_query_len, embed_dim,3)
        attn_output = self._proj_out(attn_output)
