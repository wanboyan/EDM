...
class TransformerEncoderLayer_equi_v4(nn.Module):
    ...
    def forward(self, tgt, index_pair,cnt,sh,dist_atten,
        ...
        key=self.linear_key(tgt.reshape(-1,self.d_model,3))
        value=self.linear_value(tgt.reshape(-1,self.d_model,3))
        key=attention_helper.sa_attention_mapper_v3(index_pair,key.reshape(-1,self.d_model*3),cnt).reshape(-1,neighbor_num,self.d_model,3)
        value=attention_helper.sa_attention_mapper_v3(index_pair,value.reshape(-1,self.d_model*3),cnt).reshape(-1,neighbor_num,self.d_model,3)
