...
class TransformerDecoderLayer_equi_an(nn.Module):
    ...
    def forward(self, tgt, memory,index_pair,cnt1,cnt2,
        ...
        key=self.to_k(memory.reshape(-1,self.d_model,an).permute(0,2,1)).permute(0,2,1)
        value=self.to_q(memory.reshape(-1,self.d_model,an).permute(0,2,1)).permute(0,2,1)

        key=attention_helper.ca_attention_mapper_v3(index_pair,key.reshape(-1,self.d_model*an),cnt1,cnt2).reshape(-1,neighbor_num,self.d_model,an)
        value=attention_helper.ca_attention_mapper_v3(index_pair,value.reshape(-1,self.d_model*an),cnt1,cnt2).reshape(-1,neighbor_num,self.d_model,an)
