...
def ca_attention_mapper_v3(index_pair,key_features, query_cnt,key_cnt):


    batch_size=index_pair.shape[0]//(query_cnt.item())

    query_cnt=query_cnt.repeat(batch_size)
    key_cnt=key_cnt.repeat(batch_size)

    key_attn_features = grouping_operation(
        key_features, key_cnt, index_pair, query_cnt).permute(0, 2, 1).contiguous()


    return key_attn_features

def sa_attention_mapper_v3(index_pair, query_features,query_cnt):
    batch_size=index_pair.shape[0]//(query_cnt.item())
    query_cnt=query_cnt.repeat(batch_size)
    query_attn_features = grouping_operation(
        query_features, query_cnt, index_pair, query_cnt).permute(0, 2, 1).contiguous()

    return query_attn_features

def mapper_v6(index_pair, key_features,batch_size):
    query_cnt=index_pair.shape[0]//batch_size
    key_cnt=key_features.shape[0]//batch_size

    query_cnt=torch.tensor(query_cnt).repeat(batch_size).int().to(index_pair.device)
    key_cnt=torch.tensor(key_cnt).repeat(batch_size).int().to(index_pair.device)
    key_attn_features = grouping_operation(
        key_features, key_cnt, index_pair, query_cnt).permute(0, 2, 1).contiguous()
