...
class LayerNorm(nn.Module):
    ...
    def forward(self, x):
        return F.layer_norm(x, x.shape[-1:], self.gamma, self.beta,eps=0)
...
class VNLayerNorm(nn.Module):
    ...
    def forward(self, x):
        norms = x.norm(dim = -1)
        # mean=torch.mean(norms,dim=-1,keepdim=True)
        # var=torch.mean((norms-mean)**2,dim=-1,keepdim=True)
        # new_norms=(norms-mean)/torch.sqrt(var)
