...
def my_vsd(
    ...
    gt_renderer = partial(
        pyrenderer,
        BOP_cad_trimesh=gt_mesh,
        intrinsic=cam_K,
        img_size=image_size,
    )

    pred_renderer = partial(
        pyrenderer,
        BOP_cad_trimesh=pred_mesh,
        intrinsic=cam_K,
        img_size=image_size,
    )
    # gt poses
    ...
    gt_poses=gt_pose[None,:,:]
    pred_poses=pred_pose[None,:,:]
    gt_depths = gt_renderer(gt_poses)
    pred_depths = pred_renderer(pred_poses)
    vsd_error = np.zeros(len(gt_depths))
    for idx_poses in range(len(gt_depths)):
        depth_gt = gt_depths[idx_poses]
        depth_est = pred_depths[idx_poses]
        # Convert depth images to distance images.
        dist_test = depth_im_to_dist_im_fast(depth_test, cam_K)
        dist_gt = depth_im_to_dist_im_fast(depth_gt, cam_K)*1000
        dist_est = depth_im_to_dist_im_fast(depth_est, cam_K)*1000
