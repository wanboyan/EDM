...
result_dir='/data_sata/pack/result/nocs_camera'
pred_result_save_path='/data_sata/pack/result/nocs_camera.pkl'
...
depth_path_temp='/data_nvme/nocs_data/CAMERA/val/{:05d}/{:04d}_depth.png'
...
cd_list=[]
pred_results=[]
cam_K=np.array([[577.5, 0, 319.5], [0, 577.5, 239.5], [0, 0, 1]],
               dtype=np.float)
...
for subfolder in subfolders:
    ...
    for obj_key in combine:
        ...
        if gt_id not in id2mesh:
            gt_mesh_path=os.path.join(ply_dir,f"{gt_id}.ply")
        ...
        # show_open3d(ro_match_cloud,match_cloud)
        no_re_gt_cloud_z=copy.deepcopy(no_re_gt_cloud)
        no_re_gt_cloud_z[:,2]=-no_re_gt_cloud_z[:,2]
        # show_open3d(match_cloud,no_re_gt_cloud_z)
        dist1, dist2=cd_loss(torch.from_numpy(no_re_gt_cloud_z)[None,:,:].float(),torch.from_numpy(match_cloud)[None,:,:].float())
        ...
        gt_scale_matrix=np.diag([no_re_gt_scale,no_re_gt_scale,-no_re_gt_scale,1])
        gt_mesh_pose= np.eye(4)
        gt_mesh_pose[:3,3]=no_re_gt_t
        gt_mesh_pose[:3,:3]=gt_R
        gt_mesh_pose=gt_mesh_pose @ gt_scale_matrix
        ...
        gt_mesh_cloud_cam=np.einsum('ij,pj->pi',gt_R,gt_mesh_cloud)*no_re_gt_scale+no_re_gt_t
