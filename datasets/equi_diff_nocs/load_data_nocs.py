...
class PoseDataset_nocs():
    def __init__(self, data_dir, detection_dir,mode='test',source='Real',per_obj=None,
                 n_pts=1024, img_size=256,sota_dicts=None):
        '''
        ...
        # new_img_list=random.sample(img_list,500)
        # with open('/data_sata/pack/result/small_img_list_500.txt', "w") as f:
        #     json.dump(new_img_list, f)
    ...
    def __getitem__(self,index):
        ...
        for j in range(num_instance):
            ...
            try:
                model_point=self.models[model_name]
                if cat_id==6:
                    meta=self.mug_meta[model_name]
                    t0=meta[0]
                    s0=meta[1]

                    model_point=model_point/s0-t0
            except:
                continue


            idx=np.random.choice(len(model_point),1024)
            model_point=model_point[idx]
        ...
        sota_Rs=np.array(sota_Rs)
        sota_Ts=np.array(sota_Ts)
        sota_scales=np.array(sota_scales)

        std_models=np.array(std_models)
        model_points=np.array(model_points)
        cls_vectors=torch.stack(cls_vectors,dim=0)
        ...
        data_dict['model_names'] = model_names
        ...
        data_dict['sota_Rs'] = torch.as_tensor(sota_Rs, dtype=torch.float32).contiguous()
        data_dict['sota_Ts'] = torch.as_tensor(sota_Ts, dtype=torch.float32).contiguous()
        data_dict['sota_scales'] = torch.as_tensor(sota_scales, dtype=torch.float32).contiguous()
        ...
        data_dict['cls_vectors'] = torch.as_tensor(cls_vectors, dtype=torch.float32).contiguous()
