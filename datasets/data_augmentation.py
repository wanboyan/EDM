...
def defor_2D(roi_mask, rand_r=2, rand_pro=0.3):
    ...
    mask_erode = cv2.erode(mask, kernel, iterations=5)  # rand_r
...
def dilate_mask(roi_mask, rand_r=2, rand_pro=0.3,dilate_times=None):
    '''
    ...
    #     return roi_mask
    mask = roi_mask.copy()
    if dilate_times==None:
        dilate_times = np.random.randint(0,6)
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
    mask_dilate = cv2.dilate(mask, kernel, iterations=dilate_times)
...
def defor_scale(pc, R, t, s, aug_bb):
    # pc  n x 3, here s must  be the original s
    pc_reproj = torch.mm(R.T, (pc - t.view(1, 3)).T).T  # nn x 3
    ...
    pc_reproj[:, 0] = pc_reproj[:, 0] * ex
    pc_reproj[:, 1] = pc_reproj[:, 1] * ex
    pc_reproj[:, 2] = pc_reproj[:, 2] * ex
    s = s * ex
    pc_new = torch.mm(R, pc_reproj.T) + t.view(3, 1)
    pc_new = pc_new.T
...
def defor_3D_pc(pc, r=0.05):
    ...
    pc = pc + torch.clip(points_defor * r,-0.005,0.005)
    return pc
