...
class PoseDataset(data.Dataset):
    def __init__(self, source=None, mode='train', data_dir=None,
        ...
        if per_obj=='sym':
            self.sym_obj_list=['bottle','can','bowl']
            self.sym_obj_id_list = []
            for cat_name in self.sym_obj_list:
                self.sym_obj_id_list.append(self.cat_name2id[cat_name])
        ...
        if self.per_obj=='sym':
            for k,v in models.items():
                if model_name2cat_name[k]in self.sym_obj_list:
    ...
    def __getitem__(self, index):
        ...
        if self.per_obj != '' and self.per_obj !='sym':

            idx = gts['class_ids'].index(self.per_obj_id)
            valid_indexes=[index for index,id in enumerate(gts['class_ids']) if id==self.per_obj_id]
            idx=random.choice(valid_indexes)
        elif self.per_obj =='sym':
            valid_indexes=[index for index,id in enumerate(gts['class_ids']) if id in self.sym_obj_id_list]
        ...
        if 'CAMERA' in img_path.split('/') and FLAGS.aug_bg:
            # comp_depth[mask_target==1.0]=depth[mask_target==1.0]
            if comp_depth is None:
                print(img_path+"dont have composed depth")
            else:
                depth=comp_depth
