...
def train(argv):
    ...
    train_dataset = PoseDataset(data_dir=FLAGS.dataset_dir,mode='train',per_cat=cat_name)
    ...
    if FLAGS.stage==2 and not FLAGS.resume:

        check_files=[f for f in os.listdir(os.path.join(FLAGS.stage_1_dir,cat_name)) if f.startswith('model_')]
        latest_file=max(check_files,key=lambda x: int((x.split('_')[-1]).split('.')[0]))

        resume_path=os.path.join(FLAGS.stage_1_dir,cat_name,latest_file)
        ...
        print('load stage_1',resume_path)
    ...
    if FLAGS.resume:
        ...
        weight_id=resume_path.split('/')[-1].split('.')[0].split('_')[1]
        state_dict=torch.load(resume_path)
        network.load_state_dict(state_dict)
        s_epoch = int(weight_id)
    else:
        s_epoch = 0
    ...
    # if FLAGS.resume:
    #
    #     resume_path=os.path.join(FLAGS.resume_dir,cat_name,f'optimizer_{weight_id}.pth')
    #     print('load',resume_path)
    #     state_dict=torch.load(resume_path)
    ...
    for epoch in range(s_epoch, FLAGS.total_epoch):
        ...
        for i, data in enumerate(train_dataloader, 1):
            ...
            loss_dict= network(depth=data['roi_depth'].to(device),
                               ...
                               cls_vector=data['cls_vector'].to(device),
                               occ_r_vector=data['occ_r_vector'].to(device),
                               )
            ...
            if FLAGS.use_board:
                ...
                if loss_dict['loss_100_r']>0:
                    tb_writter.add_scalar('loss_100_r',loss_dict['loss_100_r'],global_step)
                ...
                if loss_dict['loss_1000_r']>0:
                    tb_writter.add_scalar('loss_1000_r',loss_dict['loss_1000_r'],global_step)
