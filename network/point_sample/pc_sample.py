...
def PC_sample(obj_mask, Depth, camK, coor2d):
    ...
    for i in range(bs):
        ...
        if FLAGS.sample_method == 'basic':
            ...
            # p_n_now=defor_3D_pc(p_n_now,r=1)
...
def defor_3D_pc(pc, r=1):
    points_defor = torch.randn(pc.shape).to(pc.device)
    pc = pc + torch.clip(points_defor * r,-5,5)
    return pc
...
def farthest_sample(points,sample_num):
    ...
    # ).long()
    with torch.no_grad():
        vertices_pool, idx = sample_farthest_points(points.unsqueeze(0), K=sample_num)
    return idx[0]